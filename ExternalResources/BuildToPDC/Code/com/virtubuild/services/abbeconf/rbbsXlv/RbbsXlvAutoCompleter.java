...
public class RbbsXlvAutoCompleter { 
    ...
    */
    public RbbsXlvAutoCompleter(Component column, Configuration configurator) {
        this.column = column;
    ...
    */
    ...
    if (column.getStateVector().getVar(str).getLegalValues().size() == 10 || column.getStateVector().getVar(str).getLegalValues().size() == 8){
    ...
    if (column.getStateVector().getVar(str).getLegalValues().size() == 5){
	  	if( column.getStateVector().getRole(str).getValue() == StateValue.NONE )
	  		{
	  			/* Create a component for rack controller */
